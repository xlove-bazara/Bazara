...
// Handles Webhook Verification (GET) and Incoming Messages/Status Events (POST)
  ...
  if (req.method === 'POST') {
    try {
      ...
      if (!body || body.object !== 'whatsapp_business_account') {
        return res.status(404).json({ error: 'Not a WhatsApp Business Account payload' });
      }
        ...
        for (const change of changes) {
          ...
          if (value.messages && Array.isArray(value.messages)) {
            ...
            for (const msg of value.messages) {
              ...
              const msgType = msg.type; // 'text', 'image', 'document', 'audio', 'video', 'sticker', 'reaction', etc.
              ...
              if (msgType === 'text') {
                ...
                mediaUrl = `/api/whatsapp-media?media_id=${mediaId}`;
      ...
      // Meta expects 200 to acknowledge webhook receipt even on transient internal error
